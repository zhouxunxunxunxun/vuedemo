...
export default {
    ...
    methods:{
        apply(){
            this.$axios.post("http://222.24.63.61:8080/apply/add2",{apply_use:this.form.tujing,apply_day:this.form.day,apply_password:this.form.password,apply_name:this.form.applyname,name:this.form.name}).then((res)=>{
                if(res.data=="success"){
                    alert("申请成功！");
                }else{
                    alert("申请失败！");
    ...
    created(){
        this.$axios.get("http://222.24.63.61:8080/apply/query_linux").then((res)=>{
